...
def dados_produtos_siac() -> pd.DataFrame:
    with Postgres() as db: df_products_siac = db.query('select codpro, produto, codfor, codgru, codsubgru, codsec, num_fab, num_orig, embala, fantasia, in_lixeira from "D-1".produto')
    df_products_siac['embala'] = df_products_siac.embala.replace(0,1)
    return dados_produtos_siac
...
def dados_fotos() -> pd.DataFrame:
    df_photos_products = pd.read_excel('../data/photos_by_codpro_2023_05_29.xlsx', dtype=str)
    return df_photos_products

def verify_photo(
        df_merge_products_storage_photos:pd.DataFrame, df1:pd.DataFrame) -> pd.DataFrame:
    for idx in tqdm(df_merge_products_storage_photos.index): # indice de cada produto.
...
def purchase_order(
        list_consulta:list, list_serie:list=None, purchase_order:bool=False, dt_chegou_purchase_order:bool=False, dt_entrance_notes:bool=False) -> pd.DataFrame:
...
if __name__ == '__main__':
    ...
    # Quantidade de itens que possuem fotos.
    df1.query('img_fraga or img_siac').reset_index(drop=True)
    # Quantidade de itens que possuem fotos e estoque diferente de 0.
    df2 = df1.query('(img_fraga or img_siac) and (estoque != 0)').reset_index(drop=True)
    # Quantidade de itens que possuem fotos e estoque igual a 0.
    df3 = df1.query('(img_fraga or img_siac) and (estoque == 0)').reset_index(drop=True)
    ...
    df_compra = pd.merge(
        df_prod_nfc, df_compra, on=['cd_loja', 'codfor', 'serie', 'numnot'], how='left', suffixes=['_prod_nfc', '_compra']
    ).sort_values('dt_chegou', ascending=False)
    df4 = pd.merge(
        df1, df_compra, on=['codpro', 'codfor'], how='left'
    )
    ### -> QTD SKUs com pedido de compra ou em estoque no grupo que tem fotos, e estão cadastrados Vs. que não estão cadastrados.
    df5 = pd.merge(
        df4, df_ml_info, on='codpro', how='left'
    )
    # Produtos com Estoque, Pedido de compra e Cadastrado no MercadoLivre
    df5.query('(~item_id.isna()) and (img_fraga | img_siac) and (estoque != 0 | ~emissao.isna())')
    # Produtos com Estoque, Pedido de compra e que nao estao Cadastrado no MercadoLivre
    df5.query('(item_id.isna()) and (img_fraga | img_siac) and (estoque != 0 | ~emissao.isna())')
