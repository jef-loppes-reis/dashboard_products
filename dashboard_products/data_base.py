...
def verify_photo(df_photos_products:pd.DataFrame,df1:pd.DataFrame) -> pd.DataFrame:
    ...
    for idx in tqdm(df1.index, total=len(df1)):
        ...
        photo_fraga = df_photos_products.query(f'codpro == "{codpro}"').img_fraga.values[0]
        photo_siac = df_photos_products.query(f'codpro == "{codpro}"').img_siac.values[0]
...
def main():
    ...
    df_merge_products_storage_photos = df_merge_products_storage_photos.fillna({'img_fraga':False, 'img_siac':False})
